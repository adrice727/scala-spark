...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val fields = for {
      (name, i) <- columnNames.zipWithIndex
      dataType = if (i == 0) StringType else DoubleType
    } yield StructField(name, dataType, nullable = false)
    StructType(fields)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    columnNames.foldLeft((List[Column](), List[Column](), List[Column]()))((acc, a) => {
      if (isActivityType(a, primary)) (col(a) :: acc._1, acc._2, acc._3)
      else if (isActivityType(a, working)) (acc._1, col(a) :: acc._2, acc._3)
      else if (isActivityType(a, other)) (acc._1, acc._2, col(a) :: acc._3)
      else acc
    })
  }
  ...
  def timeUsageSummary(
    ...
    def isBetween(col: Column, lower: Int, upper: Int): Column = {
      col >= lower && col <= upper
    }

    val workingStatusProjection: Column =
      when(isBetween($"telfs", 1, 2), "working")
    ...
    val ageProjection: Column =
      when(isBetween($"teage", 15, 22), "young")
        .when(isBetween($"teage", 23, 55), "active")
        .otherwise("elder")
        .as("age")
